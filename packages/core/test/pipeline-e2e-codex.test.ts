/**
 * Pipeline e2e test — CLI provider (codex).
 *
 * Runs `ghostqa run` against demo-app using `codex` CLI.
 *
...
describe.skipIf(!hasCodex)("pipeline e2e — codex CLI", { timeout: 300_000 }, () => {
  let runDir: string;
  let originalConfig: string;

  beforeAll(async () => {
    originalConfig = await readFile(CONFIG_PATH, "utf-8");
    await writeFile(CONFIG_PATH, CODEX_CONFIG, "utf-8");
